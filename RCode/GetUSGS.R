...
formUSGSobject<-function(meanhaz,sdhaz){
...
ExtractUSGS<-function(url,namer,I0=NULL,plotty=F){
  ...
  sgdf<-formUSGSobject(meanhaz,sdhaz) ; rm(meanhaz,sdhaz)
...
check_hazsdf<-function(hazsdf=NULL,minmag){
...
convertUSGStime<-function(time) as.Date(as.POSIXct(time/(60*24), origin = "1984-12-20", tz = "UTC"))
...
GetUSGS_id<-function(USGSid,titlz="tmp",I0=4.5,minmag=5){
  ...
  # Don't you even bloody dare to ask why....
  sdate<-convertUSGStime(tmp$properties$time)
  print(sdate)
...
GetUSGS<-function(USGSid=NULL,bbox,sdate,fdate=NULL,titlz="tmp",I0=4.5,minmag=5){
  ...
  if(!is.null(USGSid)) {
    ...
    bbox<-hazsdf@bbox
    USGS<-SearchUSGSbbox(hazsdf@bbox,hazsdf@eventdate-7,hazsdf@eventdate+14,minmag)  
    ...
    if(lenny<=1) {
      ...
      for (i in 1:lenny){
        sdate<-min(sdate,convertUSGStime(USGS$features[[i]]$properties$time))
        fdate<-max(fdate,convertUSGStime(USGS$features[[i]]$properties$time))  
      }
    }
  ...
  lhazdat<-c(list(bbox=bbox,sdate=sdate,fdate=fdate,NumEvents=lenny,hazard="EQ",I0=I0))
  tbbox<-rep(NA,4)
  eventdates<-c()
  for (i in 1:lenny){
    ...
    if(!check_hazsdf(hazsdf,minmag)){
      lhazdat$NumEvents<-lhazdat$NumEvents-1
      next
    }
    ...
    hazsdf<-new("HAZARD",
                obj=hazsdf,
                hazard="EQ",
                dater=convertUSGStime(USGS$features[[i]]$properties$time),
                I0=I0,
                alertlevel=ifelse(is.null(tmp$properties$alert),"green",tmp$properties$alert),
                #alertscore=ifelse(i<=length(alertscores),alertscores[i],0))
                alertscore=0)
    ...
    eventdates%<>%c(hazsdf@eventdate)
  }
  if(any(is.na(tbbox))) return(NULL)
  # Modify the bounding box to fit around all hazards extracted
  lhazdat$bbox<-tbbox
  lhazdat<-c(lhazdat[1:6],eventdates=eventdates,lhazdat[7:length(lhazdat)])
  return(lhazdat)
