...
ExtractOSMbuild<-function(bbox){
  
  obj<-opq(bbox = bbox,timeout = 60)%>%add_osm_feature("building") %>%
    opq_string()%>%osmdata_sf()
  obj<-obj$osm_polygons
  obj%<>%dplyr::select(building.levels,geometry)
  obj$building.levels%<>%as.numeric()
  obj$area<-as.double(st_area(obj$geometry))
...
GetOSMbuildings<-function(BD,bbox=NULL,minnum=50,plotty=F){
